...
/* ===== Types/formatters (as in your original) ===== */
type FeatureType =
...
/* ===== Simple simulation to preload a demo ===== */
function simulateDemo() {
  // a tiny “multi-RNA” demo set with mixed classes
  const anno: Annotation[] = [
    { gene_name: "gcvB", start: 100000, end: 100180, feature_type: "sRNA", strand: "+", chromosome: "chr" },
    { gene_name: "cpxQ", start: 120000, end: 120200, feature_type: "sRNA", strand: "+", chromosome: "chr" },
    ...
    { gene_name: "sdhC_5UTR", start: 180000, end: 180120, feature_type: "5'UTR", strand: "+", chromosome: "chr" },
    { gene_name: "sodB_5UTR", start: 175000, end: 175160, feature_type: "5'UTR", strand: "+", chromosome: "chr" },
  ];
  // helper to add a bunch of pairs for a ref
  function mkPairs(ref: string, targets: string[]): Pair[] {
    return targets.map((t, i) => ({
      ref, target: t,
      counts: 10 + i * 7,
      odds_ratio: 12 + i * 20,
      totals: 1000 + i * 100,
      ref_type: anno.find(a => a.gene_name === ref)?.feature_type,
      target_type: anno.find(a => a.gene_name === t)?.feature_type,
    }));
  }
  const pairs: Pair[] = [
    ...mkPairs("gcvB", ["oppA_5UTR", "argT_5UTR"]),
    ...mkPairs("cpxQ", ["sdhC_5UTR", "sodB_5UTR"]),
  ];
  return { anno, pairs };
}
...
function exportSVG(svgId: string, filename: string) {
  ...
  style.textContent = 'text{font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;fill:#334155;font-size:11px}';
...
export default function CsMapPage() {
  ...
  const [genesInput, setGenesInput] = useState<string>("gcvB, cpxQ");
  const [sizeScaleFactor, setSizeScaleFactor] = useState<number>(1.0);
  const [sizeOffsetC, setSizeOffsetC] = useState<number>(0); // NEW: c in √(counts + c)
  ...
  // Demo button
  function loadDemo() {
    const { anno, pairs } = simulateDemo();
    setAnno(anno);
    setPairs(pairs);
    setLoadedAnnoName("(demo)");
    setLoadedPairsName("(demo)");
    setGenesInput("gcvB, cpxQ");
  }

  const geneList = useMemo(() => genesInput.split(/[, \n\t\r]+/).map((g) => g.trim()).filter(Boolean), [genesInput]);
  ...
  const { dots, totals, warnings } = useMemo(() => {
    ...
    geneList.forEach((gRaw, col) => {
      ...
      for (let i = 1; i < cand.length; i++) {
        const nx = cand[i];
        if (nx.pos > cur.pos + 1000) { peaks.push(cur); cur = nx; }
        else if (nx.or > cur.or) { cur = nx; }
      }
      ...
      for (const p of peaks) {
        ...
        const yT = symlog(y);
        const r = (Math.sqrt(Math.max(0, p.counts + sizeOffsetC)) * 1.5) * sizeScaleFactor; // NEW
        dots.push({ col, yT, r, stroke: FEATURE_COLORS[p.type] || "#F78208" });
      }
    });

    return { dots, totals, warnings };
  }, [geneList, pairs, annoByCF, sizeScaleFactor, sizeOffsetC]);
  ...
  const barY = (v: number) => bInnerH - (log10(v) / Math.log10(tMax || 1)) * bInnerH;
    ...
    <div className="mx-auto max-w-[1300px] p-4">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <input
          ...
          placeholder="Enter genes (comma/space-separated), e.g., gcvB, sroC, arrS"
          value={genesInput}
        ...
        <div className="flex items-center gap-2">
          ...
          <input type="range" min={0.1} max={2} step={0.1} value={sizeScaleFactor} onChange={(e) => setSizeScaleFactor(Number(e.target.value))} />
        </div>

        {/* NEW: √(counts + c) */}
        <div className="flex items-center gap-2">
          <div className="text-xs text-slate-700 whitespace-nowrap">Size offset c: {sizeOffsetC}</div>
          <input type="range" min={0} max={50} step={1} value={sizeOffsetC} onChange={(e) => setSizeOffsetC(Number(e.target.value))} />
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-6">
          ...
          <label className="text-sm">
            ...
            <div className="flex items-center gap-2">
              <select className="border rounded px-2 py-1 text-xs" defaultValue="" onChange={(e) => { const u = e.target.value; if (u) loadPairsFromURL(u); }}>
              ...
              <button className="border rounded px-3 py-1" type="button" onClick={() => pairsRef.current?.click()}>Choose File</button>
            </div>
          ...
          <label className="text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <button className="border rounded px-3 py-1" type="button" onClick={() => annoRef.current?.click()}>Choose File</button>
            </div>
            <div className="text-xs text-slate-500 mt-1">{loadedAnnoName || "(none loaded)"}</div>
          </label>

          {/* NEW: demo */}
          <button className="border rounded px-3 py-1 self-start" onClick={loadDemo}>Load Demo</button>
        </div>
      </div>

      {/* Collapsed scatter */}
      <div className="relative overflow-x-auto rounded-lg border bg-white">
        <button onClick={() => exportSVG("csmap-scatter", "csMAP_scatter")} className="absolute right-3 top-3 text-xs px-2 py-1 border rounded bg-white hover:bg-slate-50">
          Export SVG
        </button>
        <svg id="csmap-scatter" width={W} height={SC_H} style={{ display: "block" }}>
          <defs><style>{`text{font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;fill:#334155;font-size:11px}`}</style></defs>
          <g transform={`translate(${margin.left},${margin.top})`}>
            ...
            {geneList.map((g, i) => {
              ...
              return (
                <g key={i} transform={`translate(${cx},${innerH})`}>
                  ...
                  <text y={24} textAnchor="middle" style={{ fontStyle: disp.italic ? "italic" : "normal" }}>{disp.text}</text>
                </g>
              );
            })}
            {/* y ticks */}
            {[0, 1, 5, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000].map((v, i) => {
              ...
              const yPix = (t: number) => innerH - (t / (1 + Math.log10(5000 / 10))) * innerH;
              return (
            ...
            {dots.map((d, idx) => {
              ...
              const cy = innerH - (d.yT / (1 + Math.log10(5000 / 10))) * innerH;
              return (
        ...
        <div className="px-4 pb-4">
          <div className="mt-2 flex flex-wrap gap-4 items-center">
            ...
            <span className="ml-4 text-xs text-slate-500">Circle area ∝ √(counts{sizeOffsetC > 0 ? ` + ${sizeOffsetC}` : ""})</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative overflow-x-auto rounded-lg border bg-white mt-6">
        <button onClick={() => exportSVG("csmap-bars", "csMAP_totals")} className="absolute right-3 top-3 text-xs px-2 py-1 border rounded bg-white hover:bg-slate-50">
          Export SVG
        </button>
            ...
            {totals.map((t, i) => {
              ...
              return (
                <g key={i}>
                  ...
                  <text x={x + barW / 2} y={bInnerH + 18} textAnchor="middle" style={{ fontStyle: disp.italic ? "italic" : "normal" }}>{disp.text}</text>
                </g>
              );
            })}
          </g>
      ...
      {warnings.length > 0 && <div className="mt-3 text-xs text-amber-700">{warnings.join(" · ")}</div>}
    </div>
  );
}
